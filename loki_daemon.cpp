...
//
// -------------------------------------------------------------------------------------------------
FILE_SCOPE uint64_t amount_to_staking_portions(uint64_t amount)
{
...
bool daemon_prepare_registration(daemon_t *daemon, daemon_prepare_registration_params const *params, loki_fixed_string<> *registration_cmd)
{
  ...
  loki_fixed_string<32> owner_portions("%zu", amount_to_staking_portions(owner->amount));
  loki_fixed_string<32> owner_fee     ("%d",  params->owner_fee_percent);
  loki_fixed_string<32> owner_amount  ("%zu", owner->amount);
  ...
  if (params->num_contributors == 1)
  {
    if (params->open_pool)
    {
      itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Will the operator contribute the entire stake?"), "n");
      itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Enter operator fee as a percentage of the total staking reward"), owner_fee.str);
      itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Do you want to reserve portions of the stake for other specific contributors?"), "n");
      itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Enter the loki address for the operator"), owner->addr.buf.str);
      ...
      result &= str_match(owner_fee_output,      owner_fee.str);
      result &= str_match(owner_addr_output,     owner->addr.buf.str);
      ...
      // result &= str_match(owner_portions_output, owner_portions.str);
  ...
  else
  {
    itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Will the operator contribute the entire stake?"), "n");
    itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Enter operator fee as a percentage of the total staking reward"), owner_fee.str);
    itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Do you want to reserve portions of the stake for other specific contributors?"), "y");
    ...
    itest_read_until_then_write_stdin(&daemon->ipc, LOKI_STRING("Number of additional contributors"), num_extra_contribs_str);
    ...
    for (int i = 0; i < params->num_contributors; ++i)
    {
      ...
      result                             &= str_match(addr_output,contributor->addr.buf.str);
      char const *portions_output         = str_skip_to_next_word_inplace(&ptr);
      loki_fixed_string<32> contributor_portions("%zu", amount_to_staking_portions(contributor->amount));
      (void)portions_output; (void)contributor_portions;

      // TODO(doyle): Validate amounts, because we don't use the same portions
      // calculation as loki daemon we can be off by small amounts.
      // result &= str_match(portions_output, contributor_portions.str);
    }
...
{
  loki_fixed_string<64> cmd("ban %s", ip->str);
...
{
  itest_write_then_read_stdout_until(&daemon->ipc, "relay_votes_and_uptime", LOKI_STRING("Votes and uptime relayed"));
}
...
daemon_status_t daemon_status(daemon_t *daemon)
{
  ...
  loki_string literal = LOKI_STRING("net v");
...
{
  loki_fixed_string<256> cmd("integration_test toggle_checkpoint_quorum");
  itest_write_then_read_stdout_until(&daemon->ipc, cmd.str, LOKI_STRING("toggle_checkpoint_quorum toggled"));
}

void daemon_toggle_obligation_quorum(daemon_t *daemon)
{
  loki_fixed_string<256> cmd("integration_test toggle_obligation_quorum");
  itest_write_then_read_stdout_until(&daemon->ipc, cmd.str, LOKI_STRING("toggle_obligation_quorum toggled"));
}

void daemon_toggle_obligation_uptime_proof(daemon_t *daemon)
{
  loki_fixed_string<256> cmd("integration_test toggle_obligation_uptime_proof");
  itest_write_then_read_stdout_until(&daemon->ipc, cmd.str, LOKI_STRING("toggle_obligation_uptime_proof toggled"));
}

void daemon_toggle_obligation_checkpointing(daemon_t *daemon)
{
  loki_fixed_string<256> cmd("integration_test toggle_obligation_checkpointing");
  itest_write_then_read_stdout_until(&daemon->ipc, cmd.str, LOKI_STRING("toggle_obligation_checkpointing toggled"));
}
