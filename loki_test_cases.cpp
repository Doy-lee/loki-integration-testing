...
void print_test_results(test_result const *test)
{
  ...
  loki_fixed_string<> buf("%s ", test->name.str);
...
{
  bool              success;
  loki_fixed_string<1024> err_msg;
...
void helper_setup_blockchain_with_n_blocks(test_result const *context, daemon_t *daemon, wallet_t *wallet, loki_addr *addr, uint64_t num_blocks)
{
  ...
  *daemon                           = create_and_start_daemon(daemon_params, context->name.str);
...
test_result helper_setup_blockchain_with_1_service_node(test_result const *context, daemon_t *daemon, wallet_t *wallet, loki_addr *addr, loki_snode_key *snode_key)
{
...

  {
    ...
    loki_fixed_string<> registration_cmd = {}; // Register the service node
    ...
    wallet_register_service_node(wallet, registration_cmd.str);
                                                  ...
                                                  loki_addr *addr)
{
  ...
  {
    create_and_start_multi_daemons(daemons, num_daemons, daemon_params, num_daemons, context->name.str);
    ...
    *wallet = create_and_start_wallet(daemon_params[0].nettype, wallet_params, context->name.str);
    wallet_set_default_testing_settings(wallet);
  }
...
enum struct BanType { Unban, Ban };
void helper_ban_daemons(BanType type, daemon_t *src, daemon_t *daemons_to_ban, int num_daemons_to_ban)
{
  LOKI_FOR_EACH(daemon_index, num_daemons_to_ban)
  {
    // daemon_t const *daemon_to_ban = daemons_to_ban + daemon_index;
    loki_fixed_string<32> ban_ip("127.0.0.1");
...
test_result checkpointing__private_chain_reorgs_to_checkpoint_chain()
{
  ...
  int const NUM_SERVICE_NODES = LOKI_CHECKPOINT_QUORUM_SIZE;
  ...
  helper_ban_daemons(BanType::Ban, naughty_daemon, service_nodes, NUM_SERVICE_NODES);

  // Mine registration and become service nodes and mine some blocks to create checkpoint votes
  for (int index = 0; index < ((LOKI_VOTE_LIFETIME / LOKI_CHECKPOINT_INTERVAL) * 2); index++)
  {
  ...
  uint64_t target_naughty_daemon_height = canonical_chain_height + 128;
  {
    ...
    wallet_t naughty_wallet                   = create_and_start_wallet(daemon_params.nettype, naughty_wallet_params, result.name.str);
...
#if 0
  EXPECT(result,
         helper_compare_checkpoints(environment.service_nodes, NUM_SERVICE_NODES),
  ...
  helper_ban_daemons(BanType::Unban, naughty_daemon, service_nodes, NUM_SERVICE_NODES);
  ...
  for (int i = 0; naughty_height != target_blockchain_height; i++)
  {
    os_sleep_ms(1500);
...
#if 0
  EXPECT(result,
         helper_compare_checkpoints(environment.all_daemons.data(), NUM_SERVICE_NODES + NUM_DAEMONS),
  ...
  EXPECT(result, good_hash == naughty_hash, "%s == %s, Hashes for top block between naughty and main chain did not match. Naughty chain did NOT reorg to main chain.", good_hash.str, naughty_hash.str);
  return result;
}
...
test_result checkpointing__new_peer_syncs_checkpoints()
{
  ...
  LOKI_FOR_ITERATOR(daemon, daemons, NUM_DAEMONS)
  ...
  daemon_t *new_peer = daemons + NUM_SERVICE_NODES;
  helper_ban_daemons(BanType::Ban, new_peer, daemons, NUM_SERVICE_NODES);
  ...
  for (int index = 0; index < (LOKI_VOTE_LIFETIME / LOKI_CHECKPOINT_INTERVAL); index++)
  {
    ...
    helper_block_until_blockchains_are_synced(daemons, NUM_SERVICE_NODES);
    LOKI_FOR_ITERATOR(daemon, daemons, NUM_SERVICE_NODES)
      daemon_relay_votes_and_uptime(daemon);
    os_sleep_ms(1000);
  }

  // Unban localhost, restoring connection to the new peer and see if it syncs up
  helper_ban_daemons(BanType::Unban, new_peer, daemons, NUM_SERVICE_NODES);
  ...
  for (int i = 0; i < 100 && new_peer_height != target_blockchain_height; i++)
  {
    os_sleep_ms(2000);
...
#if 0
  EXPECT(result, helper_compare_checkpoints(daemons, NUM_DAEMONS), "Checkpoints did not match between all daemons");
#else
  // TODO(loki): Until we can enforce checkpoints are reliably transmitted, i.e.
  // I can't sync blocks without the pre-requisite checkpoints we are likely
  // going to end up with missing checkpoints since the rate at which blocks are
  // produced in tests is faster than the P2P system can keep up with.
  helper_compare_checkpoints(daemons, NUM_DAEMONS);
#endif
  ...
  EXPECT(result, good_hash == new_hash, "%s == %s, Hashes for top block between naughty and main chain did not match. Naughty chain did NOT reorg to main chain.", good_hash.str, new_hash.str);
  return result;
}
...
test_result checkpointing__deregister_non_participating_peer()
{
  ...
  {
    daemon_mine_n_blocks(miner, wallet, LOKI_CHECKPOINT_INTERVAL);
    helper_block_until_blockchains_are_synced(good_service_nodes, NUM_GOOD_SERVICE_NODES);

    LOKI_FOR_ITERATOR(node, good_service_nodes, NUM_GOOD_SERVICE_NODES)
...
test_result prepare_registration__check_all_solo_stake_forms_valid_registration()
{
  ...
  for (int i = 0; i < 101; ++i) // TODO(doyle): Assumes staking requirement
  {
    ...
    char const *register_str      = str_find(registration_cmd.str, "register_service_node");
...
test_result print_locked_stakes__check_no_locked_stakes()
{
  ...
  // Start up daemon and wallet
  {
    start_daemon_params daemon_params = {};
    daemon_params.load_latest_hardfork_versions();
    daemon                            = create_and_start_daemon(daemon_params, result.name.str);

    start_wallet_params wallet_params = {};
    wallet_params.daemon              = &daemon;
    wallet                            = create_and_start_wallet(daemon_params.nettype, wallet_params, result.name.str);
    wallet_set_default_testing_settings(&wallet);
  }
...
test_result print_locked_stakes__check_shows_locked_stakes()
{
  ...
  uint64_t const half_stake_requirement = LOKI_FAKENET_STAKING_REQUIREMENT / 2;
  {
    ...
    EXPECT(result, wallet_register_service_node(&wallet, registration_cmd.str),    "Failed to register service node");
  }
  ...
  // Check stakes show up as locked
  {
    ...
    EXPECT(result, stake->snode_key == snode_key, "Locked stake service node key didn't match what we expected, stake->snode_key: %s, snode_key: %s", stake->snode_key.str, snode_key.str);
...
test_result register_service_node__allow_4_stakers()
{
  ...
  {
    stakers[i]       = create_and_start_wallet(daemon_params.nettype, wallet_params, result.name.str);
  ...
  EXPECT(result, wallet_register_service_node(owner, registration_cmd.str), "Failed to register service node");
...
test_result register_service_node__allow_43_23_13_21_reserved_contribution()
{
  ...
  loki_snode_key snode_key = {};
  {
    ...
    loki_fixed_string<> registration_cmd = {};
    EXPECT(result, daemon_prepare_registration (&daemon, &params, &registration_cmd), "Failed to prepare registration");
    EXPECT(result, wallet_register_service_node(wallets + 0, registration_cmd.str),    "Failed to register service node");
  }
...
test_result register_service_node__disallow_register_twice()
{
  ...
  EXPECT(result, wallet_register_service_node(&wallet, registration_cmd.str) == false,
...
test_result register_service_node__check_unlock_time_is_0()
{
  ...
  loki_transaction register_tx = {};
  {
    ...
    loki_fixed_string<> registration_cmd = {};
    EXPECT(result, daemon_prepare_registration(&daemon, &registration_params, &registration_cmd), "Failed to prepare registration for Service Node");
    wallet_register_service_node(&wallet, registration_cmd.str, &register_tx);
    daemon_mine_n_blocks(&daemon, &wallet, 1);
  ...
  daemon_print_tx(&daemon, register_tx.id.str, &tx_output);

  char const *output_unlock_times_label = str_find(tx_output.c_str(), "output_unlock_times");
...
test_result request_stake_unlock__disallow_request_twice()
{
  ...
  // Prepare registration, submit register and mine into the chain
  {
    ...
    EXPECT(result, wallet_register_service_node(&wallet, cmd.str),     "Failed to register service node");
...
test_result transfer__check_fee_amount_80x_increase()
{
  ...
  start_wallet_params wallet_params = {};
  wallet_params.daemon              = &daemon;
  wallet_t src_wallet               = create_and_start_wallet(daemon_params.nettype, wallet_params, result.name.str);
  wallet_t dest_wallet              = create_and_start_wallet(daemon_params.nettype, wallet_params, result.name.str);
  ...
  // Transfer from wallet to dest
  {
    ...
    loki_transaction tx = {};
    EXPECT(result, wallet_transfer(&src_wallet, dest_addr.buf.str, 50, &tx), "Failed to construct TX");
  ...
  // Transfer from dest to wallet
  {
    ...
    loki_transaction tx = {};
    EXPECT(result, wallet_transfer(&dest_wallet, src_addr.buf.str, 45, &tx), "Failed to construct TX");
